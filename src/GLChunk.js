...
var GLCHUNK_DIM = CHUNK_DIM;
var GLCHUNK_DIM_2 = CHUNK_DIM + 2;
...
GLChunk = function(gl, chunk) {
	...
	gl.texImage2D(gl.TEXTURE_2D, 0, gl.LUMINANCE, GLCHUNK_DIM_2, GLCHUNK_DIM_2, 0, gl.LUMINANCE, gl.UNSIGNED_BYTE, null);// chunk.data);
	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
	gl.pixelStorei(gl.UNPACK_ALIGNMENT, 1)
	gl.pixelStorei(gl.PACK_ALIGNMENT, 1)
	gl.texSubImage2D(gl.TEXTURE_2D, 0, 1, 1, GLCHUNK_DIM, GLCHUNK_DIM, gl.LUMINANCE, gl.UNSIGNED_BYTE, chunk.densityData);
	...
	gl.pixelStorei(gl.UNPACK_ALIGNMENT, 1)
	gl.pixelStorei(gl.PACK_ALIGNMENT, 1)
	gl.texSubImage2D(gl.TEXTURE_2D, 0, 1, 1, GLCHUNK_DIM, GLCHUNK_DIM, gl.LUMINANCE, gl.UNSIGNED_BYTE, chunk.tileData);
	gl.bindTexture(gl.TEXTURE_2D, null);
...
GLChunk.prototype.updateBorder = function(gl, chunk, chunkNeighbor, x1, y1, x2, y2) {
	...
	var rx1 = Math.max(x1*GLCHUNK_DIM-1, x2*GLCHUNK_DIM);
	var ry1 = Math.max(y1*GLCHUNK_DIM-1, y2*GLCHUNK_DIM);
	var rx2 = Math.min((x1+1)*GLCHUNK_DIM, (x2+1)*GLCHUNK_DIM-1);
	var ry2 = Math.min((y1+1)*GLCHUNK_DIM, (y2+1)*GLCHUNK_DIM-1);
	
	// Calculate texture position:
	var textureX1 = rx1 - x1*GLCHUNK_DIM+1;
	var textureY1 = ry1 - y1*GLCHUNK_DIM+1;
	var textureX2 = rx2 - x1*GLCHUNK_DIM+1;
	var textureY2 = ry2 - y1*GLCHUNK_DIM+1;
	
	var dataX = rx1 - x2*GLCHUNK_DIM;
	var dataY = ry1 - y2*GLCHUNK_DIM;
	...
	for (var xx = 0; xx < width; ++xx) {
		for (var yy = 0; yy < height; ++yy) {
			densityData[xx+yy*width] = chunkNeighbor.densityData[dataX+xx + (dataY+yy)*GLCHUNK_DIM];
		}
	}
	...
	for (var xx = 0; xx < width; ++xx) {
		for (var yy = 0; yy < height; ++yy) {
			tileData[xx+yy*width] = chunkNeighbor.tileData[dataX+xx + (dataY+yy)*GLCHUNK_DIM];
		}
	}
