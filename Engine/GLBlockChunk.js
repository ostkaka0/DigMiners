...
GLBlockChunk.prototype.update = function(gl, gameData, blockChunk, chunkX, chunkY) {
    ...
    for (var x = 0; x < BlockChunk.dim; x++) {
        for (var y = 0; y < BlockChunk.dim; y++) {
            ...
            if (x - 1 < 0 || y - 1 < 0)
                blockIds[0] = World.blockWorld.getForeground([chunkX * BlockChunk.dim + x - 1, chunkY * BlockChunk.dim + y - 1]);
            if (y - 1 < 0)
                blockIds[1] = World.blockWorld.getForeground([chunkX * BlockChunk.dim + x, chunkY * BlockChunk.dim + y - 1]);
            if (x - 1 < 0)
                blockIds[2] = World.blockWorld.getForeground([chunkX * BlockChunk.dim + x - 1, chunkY * BlockChunk.dim + y]);
            ...
            for (var i = 0; i < 4; i++) {
                ...
                var strength = World.blockWorld.getStrength([chunkX * BlockChunk.dim + x + i % 2 - 1, chunkY * BlockChunk.dim + y + (i / 2 >> 0) - 1]);
